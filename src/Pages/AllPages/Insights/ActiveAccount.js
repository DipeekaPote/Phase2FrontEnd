...
const ActiveAccount = () => {
  ...
  const getuser = async () => {
    try {
      ...
      const response = await fetch(`${API_KEY}/common/user/email/getuserbyemail/${email}`, requestOptions);
      if (!response.ok) {
  ...
  const updatePassword = (_id, token) => {
    ...
    const urlupdateuserpassword = `${API_KEY}/common/user/password/updateuserpassword/`;
